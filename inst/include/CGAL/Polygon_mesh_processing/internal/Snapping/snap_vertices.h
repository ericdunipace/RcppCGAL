...
//
// $URL: https://github.com/CGAL/cgal/blob/v5.4.1/Polygon_mesh_processing/include/CGAL/Polygon_mesh_processing/internal/Snapping/snap_vertices.h $
...
#ifdef CGAL_PMP_SNAPPING_PRINT_RUNTIME
    timer.stop();
    Rcpp::Rcout << "time for KD Tree (parallel): " << timer.time() << std::endl;
#endif
  }
  else
...
#ifdef CGAL_PMP_SNAPPING_PRINT_RUNTIME
    timer.stop();
    Rcpp::Rcout << "time for KD Tree (sequential): " << timer.time() << std::endl;
#endif
  }
}
...
#ifdef CGAL_PMP_SNAPPING_PRINT_RUNTIME
    timer.stop();
    Rcpp::Rcout << "time for box_d (parallel): " << timer.time() << std::endl;
#endif
  }
  else
#endif // CGAL_LINKED_WITH_TBB
...
#ifdef CGAL_PMP_SNAPPING_PRINT_RUNTIME
    timer.stop();
    Rcpp::Rcout << "time for box_d (sequential): " << timer.time() << std::endl;
#endif
  }
}
...
#ifdef CGAL_PMP_SNAPPING_PRINT_RUNTIME
  timer.stop();
  Rcpp::Rcout << "time for the actual snapping (vertex-vertex): " << timer.time() << std::endl;
#endif
