...
This document will briefly cover the installation process of the header files for users since there are some possible environmental variables that can be used to aid in the download/installation of the CGAL header files on your machine.
...
```
The functions should take care of the rest! You will have the latest version of CGAL from GitHub.
...
The first, `CGAL_DIR`, will specify a directory or URL to search for the CGAL header files like above. If it is not set, the package will simply download the latest version. In the shell on Unix machines, you can do
```{bash, eval = FALSE}
...
The second variable, `CGAL_DOWNLOAD`, will let R know to install the header files automatically when installing the `RcppCGAL` package. If `CGAL_DOWNLOAd=1`, the header files will be automatically downloaded. If `CGAL_DOWNLOAD=0` or is not set at all, the header files will not be downloaded. You can set this variable via the shell
