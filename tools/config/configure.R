...
if (tar_exists) {
  define(CGAL_TEMP_TAR_PATH = path_to_tar)
}
...
DEFAULT_URL <- "https://github.com/CGAL/cgal/releases/download/v6.2/CGAL-6.2.tar.xz"
DEFAULT_VERSION <- "6.2"
...
if (tar_exists && not_set) {
  ...
  if (not_set) {
    message(
      paste0(
        "Default header bundle not found. Downloading default version ",
        DEFAULT_VERSION
      )
    )
